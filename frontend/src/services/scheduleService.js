...
export const sendTeachersScheduleService = (data) => {
  const teachersId = data.teachersId
    .map((teacherId) => `teachersId=${teacherId}`)
    .join("&"); //teachersId=65&teachersId=12
  const { semesterId, language } = data;
  axios
    .get(
      `${SEND_PDF_TO_EMAIL}/semester/${semesterId}?language=${language}&${teachersId}`
    )
    .then((response) => {
      setLoadingService(false);
      successHandler(
...
export const showAllPublicGroupsService = (id) => {
  if (id !== null && id !== undefined)
    axios
      ...
      .then((response) => {
        store.dispatch(
          showAllGroups(response.data.sort((a, b) => sortGroup(a, b)))
        );
